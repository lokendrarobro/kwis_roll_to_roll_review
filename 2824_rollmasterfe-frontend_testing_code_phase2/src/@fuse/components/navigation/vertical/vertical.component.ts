...
export class FuseVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        ...
        this._userService.machineStatus$.subscribe(machineRunningStatus => {
            this.machineStatus = machineRunningStatus;
    ...
    toggle(): void {
        if (this.machineStatus) {
            this.toastrService.warning('Machine is currently running.');
            this.close();
        }
        else {
            // Toggle
