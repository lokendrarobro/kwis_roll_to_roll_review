...
const firstPagePdf = async (robro_roll_id, logo, userName,version,total_data_count,defect_type_filter,defect_status_filter,location_filter,xLocationStart,xLocationEnd,yLocationStart,yLocationEnd,slittingData) => {
  try {
    ...
    const characterLimit = location_filter? 3000: 4000;
    ...
    let slittingAxisInfo = location_filter ? (slittingData.type == 'Width' ?
              `<tr>
                <td >
                  <b>X Roll Start mm:</b>
    ...
    if (!isNoteOverflow) {
      ...
      objs.forEach((value, index) => {
        const sliceAngle = (pieChartPercentages[index] / 100) * Math.PI * 2;
        ctx.beginPath();
        ctx.arc(120, 120, 90, startAngle, startAngle + sliceAngle); // Center (150,150), Radius 100
        ctx.lineTo(120, 120);
        ctx.fillStyle = value.colour;
        ctx.fill();
        startAngle += sliceAngle;
    ...
    if (pieChartPercentageslength > 23) {
      ...
      if (pieChartPercentageslength === 0) {
        const height = isNoteOverflow ? '300px' : '400px';
        PiechartDataContainer = ` 
         <div class="chart-container_2">
            <div class="ChartDefectList_2 flex items-center justify-center" style="min-height: ${height};">
